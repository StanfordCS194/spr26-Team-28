...
export default function FanChatScreen() {
  ...
  // Load artist name, conversation, and messages.
  const loadChat = useCallback(async () => {
    try {
      ...
      if (profile) setArtistName((profile as any).name);

      // Find existing conversation between this fan and artist.
      ...
      // Conversation may not exist yet, which is fine.
    }
  }, [artistId]);
  ...
  async function handleSend() {
    ...
    if (!convoId) {
      const { data: newConvo, error: convoError } = await supabase
        .from("conversations")
        .insert({ fan_id: userId, artist_id: artistId })
        .select("id")
        .single();
